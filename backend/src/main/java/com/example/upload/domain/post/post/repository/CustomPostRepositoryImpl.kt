...
class CustomPostRepositoryImpl(private val queryFactory: JPAQueryFactory) : CustomPostRepository {
    ...
    fun getPagedItems(pageable: Pageable, builder: BooleanBuilder?): Page<Post> {
        val postJPAQuery = queryFactory.select(QPost.post)
            .from(QPost.post)
            .where(builder)

        postJPAQuery.offset(pageable.getOffset()).limit(pageable.getPageSize().toLong())
    ...
    fun getParamBuilder(postListParamDto: PostListParamDto): BooleanBuilder {
    ...
    fun applySorting(pageable: Pageable, postJPAQuery: JPAQuery<Post>) {
        pageable.sort.stream()
            .forEach { order: Sort.Order ->
                val pathBuilder: PathBuilder<*> = PathBuilder<Post?>(Post::class.java, "post")
